...
async def start_command(message):
    data.adding_flag = 1
    await bot.send_message(message.chat.id,
                           "Пришли ссылку на страницу с "
                           "видео канала (вкладка \"Видео\")."
                           " \"back\" для отмены действия "
                           "добавления или удаления канала")
...
async def start_command(message):
    ...
    await bot.send_message(message.chat.id,
                           "Пришли название канала или ссылку "
                           "на канал, который ты хочешь исключить"
                           " \"back\" для отмены действия "
                           "добавления или удаления канала")
...
async def start_command(message):
    ...
    if len(data.final_data) == 0:
        await bot.send_message(message.chat.id,
                               "Новых видео не было",
                               parse_mode=at.ParseMode.MARKDOWN)
    else:
...
async def send_text(message):
    if message.text == "back":
        ...
        if message.text.find("https://www.youtube.com/") != -1:
            ...
            await bot.send_message(message.chat.id,
                                   "Некорректные данные. вот пример:"
                                   "https://www.youtube.com/user/psy")
        data.adding_flag = 0
    elif data.removing_flag:
        if message.text.find("https://www.youtube.com/") != -1:
            await scapers.remove_channel(
                await scapers.find_name(message.text)
            )
            await bot.send_message(message.chat.id, "Сделано")
        else:
            r = open("YT.json")
            ...
            if YT.get(message.text) is None:
                await bot.send_message(message.chat.id,
                                       "Такого канала нет в моем списке")
            else:
